...
namespace Core_proje.Areas.Writter.Controllers
{
    ...
    public class MessageController : Controller
    {
        ...
        public async Task<IActionResult> ReceiverMessage(string p)
        {
            ...
            p = values.Email;
            var messageList = writterMessageManager.GetListReceiverMessage(p);
        ...
        public async Task<IActionResult> SenderMessage(string p)
        {
            var values = await _userManager.FindByNameAsync(User.Identity.Name);
            p = values.Email;
            var messageList = writterMessageManager.GetListSenderMessage(p);
            return View(messageList);
        }
        [Route("MessageDetails/{id}")]
        public IActionResult MessageDetails(int id)
        {
            WriterMessage writerMessage = writterMessageManager.TGetByID(id);
